// main.js
const fallbackPhoto = "./WantedPhotos/487677519_9540432476064651_1164489767534795177_n.jpg";
...
function init() {
  ...
  Object.keys(schedules).forEach(n => dom.select.insertAdjacentHTML("beforeend", `<option>${n}</option>`));
...
function search({ select, slot, week, res, img }) {
  ...
  const clash = (extraBusy[name] || []).find(({ weekday, start, end }) => weekday === wIdx && !(end <= sTime || start >= eTime));

  if (!sch.data?.length) {
    res.textContent = clash ? `不能被侵犯：${clash.start}～${clash.end}` : "神秘兮兮的沒有課表，非常欠抓";
    return;
  }

  const course = sch.data[sIdx]?.[wIdx];
  res.textContent = course && course !== "0" ? `該時段有課：${course}` : clash ? `不能被侵犯：${clash.start}～${clash.end}` : "這個時間我很閒，我很欠抓！";
}

function updateMarquee({ marq }) {
  if (!Object.keys(schedules).length) { marq.textContent = "載入課表中…"; return; }
  const now = new Date(), wIdx = now.getDay() - 1, sIdx = getSlotIdx(now), t = now.toTimeString().slice(0, 5);
  if (wIdx < 0 || wIdx > 4) { marq.textContent = "週末愉快～大家都放假！"; return; }
  if (sIdx === -1) { marq.textContent = "目前不在排課節次"; return; }
  const list = freeList(wIdx, sIdx, t);
  marq.textContent = list.length ? `現在 ${slotMeta[sIdx].letter} 節有空：${list.join("、")}` : `現在 ${slotMeta[sIdx].letter} 節大家都沒空喔！`;
}
